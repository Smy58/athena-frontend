...
const titlePurchases = computed(() => purchases.value.filter((p) => p.kind === 'TITLE'))
...
<template>
  <div class="wrap">
    ...
    <h1 class="title-display" style="font-size: 1.4rem; margin-bottom: 0.3rem">📜 История покупок</h1>
    <p style="font-size: 0.82rem; color: var(--t2); margin-bottom: 1.5rem">Все звания, которые ты купил в Магазине</p>

    <div v-if="loading" class="grid" style="gap: 0.75rem">
      <SkeletonCard v-for="n in 4" :key="n" :lines="1" />
    </div>
    <div v-else-if="titlePurchases.length === 0" class="empty-state">Пока ничего не куплено</div>
    <div v-else class="grid" style="gap: 0.75rem">
      <div v-for="p in titlePurchases" :key="p.id" class="card history-row">
        <div class="history-row-top">
          <div class="title-display history-row-name">{{ p.name }}</div>
