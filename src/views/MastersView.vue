  ...
  <div class="wrap">
    <template v-if="!selected">
      ...
      <div v-else class="grid grid-cards">
        <div v-for="m in filtered" :key="m.id" class="card" style="cursor: pointer" @click="selected = m">
    ...
    <template v-else>
      ...
      <div class="auth-card" style="max-width: 560px; margin: 0 auto">
        ...
        <div class="title-display" style="font-size: 0.82rem; margin-bottom: 0.6rem">Прошедшие игры</div>
        <div v-for="g in selected.pastGames" :key="g.title" style="font-size: 0.8rem; color: var(--t2); margin-bottom: 0.3rem">{{ g.title }} — {{ g.date }}</div>
      </div>
    </template>
  </div>
</template>
